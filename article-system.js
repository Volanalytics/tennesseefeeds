...
/**
 * Comment Button to Article Link Fix
 * 
 * This script transforms comment buttons into direct links to the article page
 * rather than trying to display comments inline on the main feed.
 * 
 * Add this code at the end of your article-system.js file.
 */

(function() {
    // Function to update all comment buttons on the page
    function updateCommentButtons() {
        // Find all comment buttons on the main feed
        ...
        commentButtons.forEach(button => {
            ...
            // Get the article ID associated with this button
            const articleId = button.dataset.articleId;
            if (!articleId) return;
            
            // Create a URL for the article page
            let articleUrl;
            
            // Use ArticleSystem's createArticleUrl if available
            if (window.ArticleSystem && window.ArticleSystem.createArticleUrl) {
                // Get the article title for a nicer URL
                const titleElement = button.closest('.article-card, .bg-white')?.querySelector('h3')?.textContent || '';
                articleUrl = window.ArticleSystem.createArticleUrl(articleId, titleElement);
            } else {
                // Fallback to simple URL creation
                articleUrl = window.location.origin + window.location.pathname + '?article=' + encodeURIComponent(articleId);
            }
            
            // Replace the button's click behavior with a link
            button.addEventListener('click', function(e) {
                e.preventDefault();
                e.stopPropagation();
            ...
            // Change appearance to make it clear it's a link
            button.classList.add('comment-link');
            button.setAttribute('title', 'View and add comments');
            
            // Optional: Change icon or text if needed
            // button.innerHTML = '<i class="fas fa-comments mr-2"></i><span>Comments</span>';
        });
    }
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', updateCommentButtons);
    }
    
    // Observer to watch for newly added elements with comment buttons
    const observer = new MutationObserver(mutations => {
        let shouldUpdate = false;
        
        mutations.forEach(mutation => {
            if (mutation.type === 'childList' && mutation.addedNodes.length > 0) {
                // Check if any added nodes contain comment buttons
                mutation.addedNodes.forEach(node => {
                    if (node.nodeType === 1 && (
                        node.classList?.contains('comment-btn') || 
    ...
    // Start observing for comment buttons being added
